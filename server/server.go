...
func ListenAndServe(node string, l net.Listener, canWrite chan bool, st *store.Store, p consensus.Proposer, rwsk, rosk string) {
	var w bool
	for {
		c, err := l.Accept()
		if err != nil {
		...
		go serve(node, c, st, p, w, rwsk, rosk)
	}
}

func serve(node string, nc net.Conn, st *store.Store, p consensus.Proposer, w bool, rwsk, rosk string) {
